...
# Read files
final_data_for_analysis_x <- readRDS("Data/AVONET/final_data_for_analysis_x.RDS")
final_shapefile_clean_x <- readRDS("Data/Intermediate_Data/final_shapefile_clean_x.RDS")

## Clean up shapefile so that i can combine geometry back into final data frame
final_shapefile_clean <- final_shapefile_clean_x %>%
  dplyr::select(Park_Addre, geometry) %>%
  group_by(Park_Addre) %>%
  summarise(geometry = st_union(geometry), .groups = "drop")

final_data_with_geometry <- final_data_for_analysis_x %>%
...
gee_final_data_for_analysis_x <- final_data_for_analysis_x %>%
  left_join(ghmi_clean, by = "Park_Addre")

# Reorder season categories so that they appear correct when plotted
gee_final_data_for_analysis_x$Season <- factor(
  gee_final_data_for_analysis_x$Season,
  levels = c("Overwintering", "Spring Migration", "Breeding", "Fall Migration"),
  ordered = TRUE
)

gee_final_data_for_analysis_x$analysis <- factor(
  gee_final_data_for_analysis_x$analysis,
...
greenspaces_x <- gee_final_data_for_analysis_x %>%
  left_join(., gs_poly_df %>% dplyr::select(Park_Addre, nearest_dist_m, nearest_dist_km), by=c("Park_Addre"))
summary(greenspaces_x$nearest_dist_m)

#saveRDS(greenspaces_x, "Data/final_data_for_big_script_x.RDS")

# play with the data
ggplot(greenspaces_x, aes(x = log10(nearest_dist_m), y = species_richness, color = analysis)) +
...
hist(greenspaces_x$nearest_dist_m)
