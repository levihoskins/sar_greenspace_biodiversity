...
dist_matrix <- st_distance(greenspaces$centroid)
diag(dist_matrix) <- NA 

# Nearest neighbor distances
greenspaces$nearest_dist_m <- apply(dist_matrix, 1, min, na.rm = TRUE)
greenspaces$nearest_dist_km <- greenspaces$nearest_dist_m / 1000

# Get name of nearest neighbor
nearest_index <- apply(dist_matrix, 1, which.min)
greenspaces$nearest_neighbor <- greenspaces$Park_Addre_clean[nearest_index]

# Prepare lines connecting to nearest neighbor
greenspaces_coords <- st_coordinates(greenspaces$centroid)

lines_df <- lapply(1:nrow(greenspaces), function(i) {
  start <- greenspaces_coords[i, ]
  end <- greenspaces_coords[nearest_index[i], ]
  st_linestring(rbind(start, end))
}) %>%
...
# Build df 
nearest_table <- greenspaces %>%
  st_drop_geometry() %>%  
  dplyr::select(
    Greenspace = Park_Addre_clean,
    Nearest_Neighbor = nearest_neighbor,
    Distance_km = nearest_dist_km
  ) %>%
  mutate(Distance_km = round(Distance_km, 2)) %>% 
  arrange(Distance_km)  # sort by distance

# GT table
nearest_table_gt <- nearest_table %>%
  gt() %>%
  cols_label(
  ...
  tab_header(
    title = md("**Nearest Neighbor Distances Between Urban Greenspaces**"),
    subtitle = "Each greenspace is paired with the closest neighboring site."
  ) %>%
...
model_species <- glmmTMB(
  species_richness ~ log1p(nearest_dist_m) + (1 | nearest_neighbor),
  data = greenspace_model_data,
  family = nbinom2
)

model_migratory <- glmmTMB(
  migratory ~ log1p(nearest_dist_m) + (1 | nearest_neighbor),
  data = greenspace_model_data,
  family = nbinom2
)

model_residential <- glmmTMB(
  residential ~ log1p(nearest_dist_m) + (1 | nearest_neighbor),
  data = greenspace_model_data,
  family = nbinom2
)
...
glmm_season_migration <- glmmTMB(
  species_richness ~ log1p(nearest_dist_km) * Season * migration_status + lists +
    log1p(Park_Size_) + (1 | nearest_neighbor),
